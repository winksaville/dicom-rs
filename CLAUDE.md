...
2. **Bot session repo** (`/.bot/`): Contains the AI agent session data.
...
- Bot session repo: `.bot`
  (symlink from `~/.claude/projects/<path-to-project-root>` to `/.bot/`)
...
- `jj st -R .` / `jj st -R .bot` — show working copy status
- `jj log -R .` / `jj log -R .bot` — show commit log
...
- App body: `ochid: /.bot/<bot-chid>`
- `.bot` body: `ochid: /<app-chid>`

Use `vc-x1 chid -s code,bot -L` to capture both pre-commit
change IDs (first line app, second `.bot`).

### `.bot` cadence

`.bot` commits **per sub-step alongside the app repo**.
Each sub-step's `.bot` commit captures the session state at
that moment.

Alternative considered (`.bot` accumulates across the cycle
...
**Do not create or maintain `.bot` bookmarks that mirror
app-side branches.** This was tried once during 0.41.1-6.7 on
the impression that an app-side fork needed a `.bot`
partner; the partner bookmark went unused for the cycle and
the work landed on `.bot main` regardless. Keeping such
...
bookmarks → pushes app → finalizes `.bot`. Failures inside the
...
finalize — so dev markers land on the remote and in `.bot` as
...
placeholder. **`.bot` always pushes its `main`** regardless
of the app-side bookmark passed (see
[`.bot` cadence](#bot-cadence) — `.bot` is a linear
journal).
...
`.bot` is also mutable via this pattern when needed, though
push's `finalize` stage normally handles trailing session
writes so you rarely hit this case there.

### Manual finalize fallback

If push exited before `finalize` (e.g. `--no-finalize`
was set, or a failure between `push-app` and `finalize`),
run finalize by hand:

```
vc-x1 finalize --repo .bot --squash --push <bookmark> --delay 10 --detach --log /tmp/vc-x1-finalize.log
```
